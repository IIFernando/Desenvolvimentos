def nfts_Gravatai():
  df = pd.read_excel('/content/drive/Shareddrives/Finance 3.0/1. Tax/Fiscal Indiretos/FILIAL L4B 0001 SP - ATIVA/DMS/Importação em lote planilha/SAP Novo v1.1.xlsx', dtype=str, sheet_name='Filial Gravatai')
  ...
  if codFilial == '0048':
      filialCnpj = '24217653005316'

  #Seleção Filial e filtros.
  df = df.loc[df['Centro'] == codFilial]
  df = df.loc[df['Cidade'] != 'GRAVATAI']
  ...
  df['Local da Prestação'] = df['Local da Prestação'].str.pad(width=30, side='left', fillchar=' ')
  ...
  RG30 = pd.concat([df['T_Registro30'], df['Sep'], df['T_PessoaP'], df['Sep'], df['CNPJ'], df['Sep'],
                    ...
                    df['Complemento'], df['Sep'], df['Bairro'], df['Sep'], df['Local da Prestação'], df['Sep'],
  ...
  result.to_csv(f'/content/drive/Shareddrives/Finance 3.0/1. Tax/Fiscal Indiretos/FILIAL L4B 0001 SP - ATIVA/DMS/Importação em lote planilha/Arquivos TXT/{codFilial}.txt', index=False, header=False)