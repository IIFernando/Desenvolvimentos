...
def assinar_xml(xml_bytes, pfx_path, pfx_password):
    ...
    signer = XMLSigner(
        ...
        # use allow_weak_hashes=True se precisar de SHA1
    )

    # Monta cadeia de certificados (mesmo que só tenha um)
...
def enviar_nfts(xml_assinado, pfx_path, pfx_password):
    session = Session()
    session.mount("https://", Pkcs12Adapter(pfx_path=pfx_path, pfx_password=pfx_password))
...
if __name__ == "__main__":
    if not pfx_path or not pfx_password:
        raise ValueError("Variáveis de ambiente PFX_PATH ou PFX_PASSWORD não definidas.")
