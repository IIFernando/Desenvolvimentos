...
df = pd.read_excel('C:\\Users\\fernando.araujo_logg\\Downloads\\Consulta.xlsx', dtype=str)

df['CNPJ'] = df['CNPJ'].str.replace('.', '').str.replace('/', '').str.replace('-', '')

print('Consultando...')

with open('C:\\Users\\fernando.araujo_logg\\Downloads\\Resultado_Consulta.csv', 'a', newline='', encoding='ISO-8859-1') as arquivo:
    for c in df['CNPJ']:
      ...
      try:
        if len(c) > 11:
            ...
            arquivo.write(c + '|' + 'Erro ao consultar')
            arquivo.write(str('\n'))
