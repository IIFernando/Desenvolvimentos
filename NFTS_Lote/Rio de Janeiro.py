...
cod_rj = pd.read_excel('G:\\Drives compartilhados\\Finance 3.0\\1. Tax\\Fiscal Indiretos\\FILIAL L4B 0001 SP - ATIVA\DMS\\Importação em lote planilha\\Arquivos\\CODRJ.xlsx', dtype=str)
...
df = pd.merge(df, cod_rj, on='LC 116/2003', how='inner')
...
df.loc[(df['LC 116/2003'] == '0705') | (df['LC 116/2003'] == '0702'), 'N_OBRA'] = 'CO1'
...
df['LC 116/2003'] = df['LC 116/2003'].str.pad(width=4, side='left', fillchar='0')
...
registros = pd.concat([df['T_Registro'], df['T_Documento'], df['Serie'], df['Nota'], df['Data documento'],
                       ...
                       df['C_RESERVADO23'], df['o_Simples'], df['LC 116/2003'], df['C_RESERVADO26'], 
                       df['Beneficio'], df['CÓDIGO'], df['%ISS'], df['Valor_Serv'], df['vDeducao'], 
...
with open('C:\\Users\\logger\\Downloads\\NFTS\\NFTS_RJ.txt', 'w', encoding='utf-8') as file:
